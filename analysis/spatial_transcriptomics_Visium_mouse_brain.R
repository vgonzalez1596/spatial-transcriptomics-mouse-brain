...
# Next, we can create a heatmap containing the top marker genes per cluster to determine 
# how distinct clusters are from each other and see if they are biologically distinct. 
# First, ensure we're back in the full dataset assay 
DefaultAssay(mouse_brain) <- "Spatial.008um"
Idents(mouse_brain) <- "seurat_cluster.projected"
# Optionally, can subset the dataset to downsample and increase marker discovery speed.
mouse_brain_subset <- subset(mouse_brain, cells = Cells(mouse_brain[["Spatial.008um"]]), downsample = 1000)
DefaultAssay(mouse_brain_subset) <- "Spatial.008um"
Idents(mouse_brain_subset) <- "seurat_cluster.projected"
# We can utilize a cluster tree to order clusters by similarity for the heatmap
mouse_brain_subset <- BuildClusterTree(mouse_brain_subset, assay = "Spatial.008um", reduction = "full.pca.sketch", reorder = TRUE)
markers <- FindAllMarkers(mouse_brain_subset, assay = "Spatial.008um", only.pos = TRUE)
# Identify top 5 markers per cluster for heatmap
top5 <- markers %>%                         
  group_by(cluster) %>%
  dplyr::filter(avg_log2FC > 1) %>%
  slice_head(n = 5) %>%
  ungroup()
mouse_brain_subset <- ScaleData(mouse_brain_subset, assay = "Spatial.008um", features = top5$gene)
p_heatmap <- DoHeatmap(mouse_brain_subset, assay = "Spatial.008um", features = top5$gene, size = 2.5) + 
  theme(axis.text = element_text(size = 6)) + NoLegend()
p_heatmap

print(p_heatmap)
save_plot_png(
  "results/figures/heatmap_top5_markers_per_cluster.png",
  p_heatmap, width = 4200, height = 3000, res = 300)
